...
const associateHistoryController_1 = require("../controllers/associateHistoryController");
const prisma = new client_1.PrismaClient();
const router = (0, express_1.Router)();
// Validation schemas
const createAssociateSchema = zod_1.z.object({
    type: zod_1.z.enum(['SUPPLIER', 'BUYER', 'BOTH']),
    name: zod_1.z.string().min(1, "Name is required"),
    contactPerson: zod_1.z.string().optional().or(zod_1.z.literal('')),
    ...
    panNumber: zod_1.z.string().optional().or(zod_1.z.literal('')),
    ...
    joinedDate: zod_1.z.string().datetime().optional(), // Make optional for client to omit
...
const contactSchema = zod_1.z.object({
    ...
    phone: zod_1.z.string().min(1).optional().or(zod_1.z.literal('')),
    isPrimary: zod_1.z.boolean().default(false),
});
const contractSchema = zod_1.z.object({
    startDate: zod_1.z.string().datetime(),
    endDate: zod_1.z.string().datetime().optional().or(zod_1.z.literal('')),
    contractDuration: zod_1.z.string().optional().or(zod_1.z.literal('')),
    status: zod_1.z.string().default("ACTIVE"),
    notes: zod_1.z.string().optional().or(zod_1.z.literal('')),
});
const documentSchema = zod_1.z.object({
    contractId: zod_1.z.string().optional().or(zod_1.z.literal('')),
    associateId: zod_1.z.string().optional().or(zod_1.z.literal('')),
    name: zod_1.z.string().min(1),
...
const communicationSchema = zod_1.z.object({
    type: zod_1.z.enum(["MEETING", "CALL", "EMAIL", "NOTE"]),
...
const updateAssociateSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, "Name is required").optional(),
    contactPerson: zod_1.z.string().optional().or(zod_1.z.literal("")),
    email: zod_1.z.string().email("Invalid email address").optional().or(zod_1.z.literal("")),
    phone: zod_1.z.string().optional().or(zod_1.z.literal("")),
    address: zod_1.z.string().optional().or(zod_1.z.literal("")),
    type: zod_1.z.enum(["SUPPLIER", "BUYER", "BOTH"]).optional(),
    gstNumber: zod_1.z.string().optional().or(zod_1.z.literal("")),
    panNumber: zod_1.z.string().optional().or(zod_1.z.literal("")),
    creditLimit: zod_1.z.number().optional(),
    currentBalance: zod_1.z.number().optional(), // Changed to optional for updates
    status: zod_1.z.enum(['ACTIVE', 'INACTIVE']).optional(),
    notes: zod_1.z.string().optional().or(zod_1.z.literal('')),
    joinedDate: zod_1.z.string().datetime().optional().or(zod_1.z.literal('')),
    paymentTerms: zod_1.z.string().optional().or(zod_1.z.literal('')),
});
...
router.get('/', async (req, res) => {
    try {
        ...
        if (include.includes('contacts'))
            includeOptions.contacts = true;
        if (include.includes('transactions'))
            includeOptions.transactions = true;
        if (include.includes('communications'))
            includeOptions.communications = true;
        ...
        if (include.includes('documents'))
            includeOptions.documents = true;
        if (include.includes('purchaseOrders'))
            includeOptions.purchaseOrders = true; // Added purchaseOrders
...
router.get('/:id/analytics', async (req, res) => {
    try {
        const analytics = await (0, analyticsController_1.getAssociateAnalytics)(req.params.id);
        res.json(analytics);
    }
    catch (error) {
    ...
    try {
        ...
        if (req.params.id) {
            ...
            if (associate.type === "BUYER" || associate.type === "BOTH") {
                const history = await (0, associateHistoryController_1.getCustomerPurchaseHistory)(req.params.id);
    ...
    try {
        if (req.params.id) {
            ...
            if (associate.type === "SUPPLIER" || associate.type === "BOTH") {
                const supplierInfo = await (0, associateHistoryController_1.getSupplierInformation)(req.params.id);
...
// Analytics routes
router.get('/:associateId/analytics', analyticsController_1.getAssociateAnalytics);
router.get('/:associateId/customer-history', associateHistoryController_1.getCustomerPurchaseHistory);
router.get('/:associateId/supplier-info', associateHistoryController_1.getSupplierInformation);
...
router.post('/export', async (req, res) => {
    try {
        ...
        const rows = data.map(associate => headers.map(header => {
            let value = associate[header];
