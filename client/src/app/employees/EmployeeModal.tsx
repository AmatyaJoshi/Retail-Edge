...
const EmployeeModal: React.FC<EmployeeModalProps> = ({ employee, isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 ${isDarkMode ? 'dark' : ''}`} onClick={onClose}>
      <div className={`rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col border ${isDarkMode ? 'bg-gray-900 border-gray-700 text-gray-100' : 'bg-white border-gray-200 text-gray-900'}`} onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className={`flex items-center justify-between p-6 border-b sticky top-0 rounded-t-2xl ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 scrollbar-thumb-rounded-full scrollbar-track-rounded-full dark:scrollbar-thumb-gray-700 dark:scrollbar-track-gray-900 p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-lg p-4 md:col-span-2`}>
        ...
        <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
          <button
            onClick={onClose}
            className={`px-4 py-2 text-sm font-medium ${isDarkMode ? 'text-gray-800 bg-gray-100 border border-gray-300' : 'text-gray-800 bg-gray-100 border border-gray-300'} rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200`}
          >
            Close
          </button>
          {isEditing ? (
            <>
              <button
                onClick={handleSave}
                disabled={saving}
                className={`px-4 py-2 text-sm font-medium ${isDarkMode ? 'text-white bg-blue-700 border border-blue-700' : 'text-white bg-blue-700 border border-blue-700'} rounded-md hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200`}
              >
                {saving ? 'Saving...' : 'Save'}
              </button>
              <button
